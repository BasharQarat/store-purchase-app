...
**Vendored dependencies, not npm/CDN.** `vendor/jsbarcode/` and `vendor/zxing/` are pre-built UMD bundles loaded as plain `<script>` tags in `index.html` (before the `type="module"` app script), so code references them as bare globals (`JsBarcode(...)`, `ZXing.BrowserMultiFormatReader`) rather than importing them — this is required for the service worker to cache the app fully offline, and there's no CDN dependency at runtime. `js/scan.js` wraps `ZXing.BrowserMultiFormatReader` for camera decoding — this deliberately replaced the native `BarcodeDetector` API, which on Android silently fails to detect anything on phones without a working Google Play Services ML Kit module (e.g. Tecno/Transsion devices) while reporting itself as supported.
